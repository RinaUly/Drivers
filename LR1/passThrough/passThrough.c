...
--*/
{
    ...
    // Вывод имени файла (FileName в UNICODE_STRING)
    ...
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"mednikova");

    // Ничего не выводим, чтобы не загружать оперативную память
    if (!NT_SUCCESS(status))
    {
        //DbgPrint("[-] PassThrough, FltGetFileNameInformation failed, FileName = %wZ\n",
            //Data->Iopb->TargetFileObject->FileName);
...
--*/
{
    //начало
    NTSTATUS status;
    PFLT_FILE_NAME_INFORMATION NameInfo = NULL; // информация о файле
    //вызываем функцию, которая возвращает нам информацию о файле
    status = FltGetFileNameInformation(
        Data,
        FLT_FILE_NAME_NORMALIZED |
        FLT_FILE_NAME_QUERY_DEFAULT,
        &NameInfo); // парсинг имени файла на составляющие
    // Определение расширения, Перевод расширения в UNICODE_STRING
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"yakim"); // юникод-строка с названием расширения
    ...
    if (!NT_SUCCESS(status))
    {
    ...
    else if ( // проверка на совпадение расширения с шаблоном
        RtlEqualUnicodeString(
            &required_extension,
            &NameInfo->Extension, \
            FALSE)) {
        ...
        //проверка действия ЛИБО на запись, ЛИБО на чтение
        //проверка на ЗАПИСЬ IRP_MJ_WRITE
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) { //проверяем, что в буфере что-то есть
                ...
                uint8_t hexarray[1024];  //определяем массив, в котором будет храниться буфер
                ...
                // -----шифрование
                //определение ключа
                unsigned char k[] = "1234567890ABCDEF";    //16 bits
                uint8_t* key = (uint8_t*)k; //приведение ключа к нужному формату/виду
                //вектор инициализации
                uint8_t iv[] = { 0x75, 0x52, 0x5f, 0x69, 0x6e, 0x74, 0x65, 0x72, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67, 0x21, 0x21 };

                struct AES_ctx ctx; //определение структуры

                AES_init_ctx_iv(&ctx, key, iv); //вызов функции у структуры 

                //вызов функции шифрования
                AES_CBC_encrypt_buffer(&ctx, hexarray, 1024);//шифруем буфер (передаем контекст, буфер и размер буфера)
        ...
        //проверка на чтение
            ...
            DbgPrint("Read Buffer: %s\n", (char*)Data->Iopb->Parameters.Read.ReadBuffer); //вывод содержимого буфера
            ...
            if (strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer) != 0) {
                ...
                struct AES_ctx ctx; // инициализация структуры

                AES_init_ctx_iv(&ctx, key, iv);
                AES_CBC_decrypt_buffer(&ctx, hexarray, 1024); //расшифровываем буфер 
                DbgPrint("Result:");
